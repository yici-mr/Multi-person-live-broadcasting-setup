...
function selectgoodid() {
    ...
    $('#price').html('<img src="' + static_url + '/static/link/2017lanse/images/load.gif" />');
    $.post('/ajax/getgoodinfo', {goodid: goodid}, function (data) {
        if (data) {
			...
			$('[name=is_user_sxf]').val(data.is_user_sxf);
			$('[name=is_user_sxffy]').val(data.is_user_sxffy);
            if (data.is_discount == 1){
                var fav = " 商品原价：" + data.price + "元<br/>批发价格：<br />";
                $.post('/ajax/getdiscountdetails', {goodid: goodid}, function (data) {
                    fav += data;
                    $("#discountdetail").html(fav);
                });
                $("#isdiscount_span").show();
            } else {
                $("#isdiscount_span").hide();
                $("#discountdetail").hide();
                $("#discountdetail").html("");
            }
            getrate();
...
function verify_pwdforbuy() {
    ...
    var goodid = $('#goodid').val();
    $.post('/ajax/checkpwdforbuy', {pwdforbuy: pwdforbuy, goodid: goodid, t: new Date().getTime()}, function (data) {
        if (data == 'ok') {
...
function goodschk() {
	...
	var temp = $('#is_user_sxffy').val();
	var zsxf = 0;
	
	
	// if($('#is_user_sxf').val()==1){
		
		// zsxf = parseFloat(tprice * 0.02);
		// tprice =  tprice + zsxf;
		// zsxf = zsxf.toFixed(2);
	// }
	...
	// if( zsxf!=0 && $('#is_user_sxf').val() == 1)
	// {
		// $('.tprice').html(tprice + '元   （手续费 ' + zsxf +'元）');
	// }else
	// {
		// $('.tprice').html(tprice);
	// }
	
	$('.tprice').html(tprice+ '元');
    $('.gprice').html(gprice);
...
function getCardLength() {
    ...
    $.post('/ajax/getcardlength', {channelid: pid}, function (data) {
        if (data) {
    ...
    $('#submit').click(function () {
       if ($('[name=isagree]').is(':checked') == false) {
        ...
        if (isNaN(quantity) || quantity <= 0 || quantity == '') {
		 ...
		 if (parseInt(quantity) > parseInt(is_xiangou) &&parseInt(is_xiangou)!=0) {
            alert('最多可以购买'+is_xiangou+'件！');
            $('[name=quantity]').focus();
            return false;
        } 
        ...
        if (kucun > 0 && quantity > kucun) {
		 ...
		 var paymoney = $('[name=danjia]').val();
	
		 if((paymoney*quantity) < 0.01)
		 {
			 alert('商品应付总额必须大于 0.3元 ');
			  $('[name=danjia]').focus();
            return false;
		 }
		...
		if (contact == '123456' ||contact == '1234567' ||contact == '12345678' ||contact == '123456789' ||contact == '111111'||contact == '222222'||contact == '333333' ||contact == '123123' ||contact == '666666' ||contact == '888888') 
		{
			alert('联系方式填写过于简单,请修改谢谢');
            $('[name=contact]').focus();
            return false;
		}
		
		
        // var arr = [123456,654321,111111,222222,333333,123456789,987654321,112233,111222,111222333];
      	if (contact == '' ) {
            alert('请填写联系方式，必须填写哦');
            $('[name=contact]').focus();
            return false;
        ...
        // if ($.inArray(contact, arr)){
        //     alert('此联系方式不可用，推荐填写自己的QQ号！');
        //     $('[name=contact]').focus();
        //     return false;
        // }
